...
/**
 * キャッシュ保持設定を更新する UseCase。
 *
 * 本 UseCase は、Sample Feature の「キャッシュを保持する」設定を更新する
 * アプリケーション操作を表す。
 *
 * ■ 提供する責務
 *   キャッシュ保持設定更新の入口
 ...
 */
class UpdateSampleKeepCacheEnabledUseCase @Inject constructor(
    ...
    suspend operator fun invoke(enabled: Boolean): AppResult<Unit> {
        return repository.setKeepCacheEnabled(enabled)
    }
}