...
/**
 * Sample Settings 画面の Route Composable。
 *
 * 本 Composable は、Navigation / ViewModel / Drawer / Snackbar / Dialog と
 ...
 *   Snackbar 表示
 *   Dialog 表示状態の管理
 *   SampleSettingsScreen への State / Event 接続
 *
 * ■ 設計上の意図
 *   SampleSettingsScreen を純粋な表示 Composable として保つため、
 *   ViewModel、NavController、DrawerState、SnackbarHostState、DialogState への依存は
 *   Route 側に閉じ込める。
 *
 ...
 *
 * ■ 注意
 *   Snackbar / Dialog は一回性の UiEffect を受けた Route 側で実行する。
 *   これらを UiState に含めないことで、State 復元や再描画による
 *   意図しない再表示を避ける。
 *
 * @param navController 画面遷移を制御する NavController。
 * @param viewModel Sample Settings 画面の ViewModel。
 */
@Composable
fun SampleSettingsRoute(
    navController: NavController,
    viewModel: SampleSettingsViewModel = hiltViewModel()
) {
    ...
    LaunchedEffect(viewModel) {
        launch(start = CoroutineStart.UNDISPATCHED) {
            viewModel.uiEffect.collect { effect ->
                when (effect) {
                    ...
                    is SampleSettingsUiEffect.ShowSnackbar -> {
        ...
        viewModel.sendEvent(SampleSettingsUiEvent.Initialize)
    }
                    ...
                    onItemClick = { item ->
                        when (item) {
                            ...
                            SampleBottomNavigationItem.Home -> {
                                viewModel.sendEvent(SampleSettingsUiEvent.HomeClicked)
                            }
