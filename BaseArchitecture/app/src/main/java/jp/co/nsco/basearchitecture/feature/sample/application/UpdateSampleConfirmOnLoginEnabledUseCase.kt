...
/**
 * ログイン時確認設定を更新する UseCase。
 *
 * 本 UseCase は、Sample Feature の「ログイン時に確認する」設定を更新する
 * アプリケーション操作を表す。
 *
 * ■ 提供する責務
 *   ログイン時確認設定更新の入口
 ...
 */
class UpdateSampleConfirmOnLoginEnabledUseCase @Inject constructor(
    ...
    suspend operator fun invoke(enabled: Boolean): AppResult<Unit> {
        return repository.setConfirmOnLoginEnabled(enabled)
    }
}